...
// The zCCH block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCCH is a hash of the serial
...
{
    //Create an output returning the zCCH that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCCHOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCCH output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCCH", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCCHOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCCH output", __func__);
...
{
    CzCCHTracker* zcchTracker = pwallet->zcchTracker.get();
    CMintMeta meta;
    if (!zcchTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcchTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CCH stake is the outpoint
