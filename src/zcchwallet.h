...
#ifndef Chaincash_ZCCHWALLET_H
#define Chaincash_ZCCHWALLET_H
...
public:
    CzCCHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCCH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCCH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Chaincash_ZCCHWALLET_H
