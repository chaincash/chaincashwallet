...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered chaincash address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CCH.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal chaincash:URI (chaincash:XyZ...?message=example)
            {
...
static void NotifyzCCHReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCCHReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCCHReset.connect(boost::bind(NotifyzCCHReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCCHReset.disconnect(boost::bind(NotifyzCCHReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcchTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
