...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CCH);
    ...
    if (!settings.contains("nAnonymizeChaincashAmount"))
        settings.setValue("nAnonymizeChaincashAmount", 1000);

    nAnonymizeChaincashAmount = settings.value("nAnonymizeChaincashAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeChaincashAmount"))
        SoftSetArg("-anonymizechaincashamount", settings.value("nAnonymizeChaincashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in chaincash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeChaincashAmount:
            return QVariant(nAnonymizeChaincashAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeChaincashAmount:
            nAnonymizeChaincashAmount = value.toInt();
            settings.setValue("nAnonymizeChaincashAmount", nAnonymizeChaincashAmount);
            emit anonymizeChaincashAmountChanged(nAnonymizeChaincashAmount);
            break;
        case CoinControlFeatures:
