...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CCH at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCCH send
enum ZerocoinSpendStatus {
    ZCCH_SPEND_OKAY = 0,                            // No error
    ZCCH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCCH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCCH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCCH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCCH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCCH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCCH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCCH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCCH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCCH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCCH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCCH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCCH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCCH_SPENT_USED_ZCCH = 14,                      // Coin has already been spend
    ZCCH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCCH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZCCHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzcchAuto) const;
    ...
    */
    ...
    CzCCHWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCCHTracker> zcchTracker;
    ...
    void setZWallet(CzCCHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcchTracker = std::unique_ptr<CzCCHTracker>(new CzCCHTracker(strWalletFile));
    }

    CzCCHWallet* getZWallet() { return zwalletMain; }
    ...
    /** zCCH reset */
    boost::signals2::signal<void()> NotifyzCCHReset;
