...
CzCCHWallet::CzCCHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcch seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCCHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCCHSeed_deprecated()) {
                    LogPrintf("%s: Updated zCCH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcch seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcch wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCCHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCCHCount(nCountLastUsed);
    else if (!walletdb.ReadZCCHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCCHWallet::Lock()
{
    seedMaster = 0;
}

void CzCCHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCCHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCCH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCCHWallet::LoadMintPoolFromDB()
{
...
void CzCCHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCCHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCCHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcchTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCCHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCCH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcchTracker which also adds to database
    pwalletMain->zcchTracker->Add(dMint, true);
...
void CzCCHWallet::SeedToZCCH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCCHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCCHWallet::GenerateDeterministicZCCH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCCHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCCHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
