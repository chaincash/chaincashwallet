...
public:
    ...
    bool WriteZCCHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCCHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCCHSeed_deprecated(uint256& seed);
    bool EraseZCCHSeed();
    bool EraseZCCHSeed_deprecated();

    bool WriteZCCHCount(const uint32_t& nCount);
    bool ReadZCCHCount(uint32_t& nCount);
